...
namespace csmatio.io
{
	...
	public class MatFileReader
	{
		...
		/// MAT-file header
		/// </summary>
		protected MatFileHeader? _matFileHeader = null;
		/// <summary>
		/// Contianer for read <c>MLArray</c>s
		/// </summary>
		readonly Dictionary<string, MLArray>? _data = null;
		//		/// <summary>
		...
		/// Array name filter
		/// </summary>
		protected readonly MatFileFilter? _filter = null;
		...
		/// <summary>
		/// Creates instance of <c>MatFileReader</c> and reads MAT data in byte[]
		/// added by Gyomei Tei in Redlin
		/// </summary>
		/// <param name="data">MAT data in byte[]</param>
		public MatFileReader(byte[] data) :
			this(data, new MatFileFilter())
		{
		}

		/// <summary>
		/// constructor, but not for read
		/// </summary>
		...
		/// <summary>
		/// Creates instance of <c>MatFileReader</c> and reads MAT data in a byte[]
		/// added by Gyomei Tei in Redlin
		/// </summary>
		/// <param name="data">all MAT data</param>
		/// <param name="filter">a MAT file filter</param>
		public MatFileReader(byte[] data, MatFileFilter filter)
		{
			_filter = filter;
			_data = new Dictionary<string, MLArray>();
			...
			try
			{
				dataIn = new MemoryStream(data,false);
			}
			catch (FileNotFoundException)
			{
				throw new MatlabIOException("Could not have a stream from a byte[] for reading!");
			}
			...
			{
				throw new MatlabIOException("Error in reading MAT data in byte[] :\n" + e);
			}
			finally
			{
				dataIn.Close();
		...
		/// </summary>
		public uint CrcOfCompressedData { get; set; } = 0;
		...
		protected Stream Inflate(Stream buf, int numOfBytes)
		{
			if ((buf.Length - buf.Position < numOfBytes) ||
				(numOfBytes<=6))	//added by Gyomei Redlen 2021/9/1
			{
				throw new MatlabIOException("Compressed buffer length miscalculated!");
			}

#if false
			// test code! save all compressed data. checked by Gyomei, Redlen
...
#endif
			//try speed up. by Gyomei Redlen 2021/9/1
			try
			{
				// skip CRC (at end) and zip format (0x789C at begin)
				//buf.Position += 2;
				buf.Seek(buf.Position + 2, SeekOrigin.Begin);
				numOfBytes -= 6;

				byte[] btTemp = new byte[numOfBytes];
				int nReadSize = 0;
				if((nReadSize = buf.Read(btTemp, 0, numOfBytes))==numOfBytes)
				{
					//read CRC
					byte[] btCRC = new byte[4];
					if(buf.Read(btCRC,0,4)==4)
					{
						if(!BitConverter.IsLittleEndian)
						{//big endian
							Array.Reverse(btCRC);
						}
						CrcOfCompressedData = BitConverter.ToUInt32(btCRC);
						using(MemoryStream ms = new MemoryStream(btTemp))
						{
							using(var df = new DeflateStream(ms, CompressionMode.Decompress))
							{
								MemoryStream mfRcd = new MemoryStream();
								df.CopyTo(mfRcd);
#if false
				// test code! save all decompressed data. added by Gyomei, Redlen
				File.WriteAllBytes("DecompressedMatlabData.bin", mfRcd.ToArray());
#endif
								mfRcd.Seek(0, SeekOrigin.Begin);
								return mfRcd;
							}
						}
					}
					...
					{
						throw new MatlabIOException($"Can not read CRC of Compressed data!");
					}
				}
				else
				{
					throw new MatlabIOException($"Compressed data size must be {numOfBytes} bytes, but get {nReadSize} bytes!");
				}
			}
			//old code
//			try
...
//#if false
//				// test code! save all decompressed data. added by Gyomei, Redlen
//				File.WriteAllBytes("DecompressedMatlabData.bin", decompressedStream.ToArray());
//#endif
...
//			}
		...
		protected virtual void ReadData(Stream buf)
		{
			// read data
			...
			switch (tag.Type)
			{
				...
				case MatDataTypes.miMATRIX:
					...
					var element = (MLArray)ReadMatrix(buf, true);
		...
		/// Modified this to virtual (2021/9/28 by Gyomei)
		/// </summary>
		/// <remarks>
		...
		protected virtual object ReadMatrix(Stream buf, bool isRoot)
		{
			// result
			...
			switch (type)
			{
				case MLArray.mxSTRUCT_CLASS:
					...
					//use tag.Padding instead of this  by Gyomei, 2021/9/1
					...
					//use seek(pos, SeekOrigin.Begin) should be more fast. by Gyomei Redlen 2021/9/1
					//br.ReadBytes(tag.Padding);
					if (tag.Padding > 0)
					{
					...
					for (var index = 0; index < mlStruct.M * mlStruct.N; index++)
					{
						for (var i = 0; i < numOfFields; i++)
						{
							...
							if (tag.Size > 0)
							{
								var fieldValue = (MLArray)ReadMatrix(br.BaseStream, false);
				...
				case MLArray.mxCELL_CLASS:
					...
					for (var i = 0; i < cell.M * cell.N; i++)
					{
						...
						if (tag.Size > 0)
						{
							var cellmatrix = (MLArray)ReadMatrix(buf, false);
							cell[i] = cellmatrix;
						}
						else
						{
		...
		protected string ZeroEndByteArrayToString(byte[] bytes)
		{
		...
		protected int[] ReadFlags(Stream buf)
		{
			var tag = new ISMatTag(buf);
		...
		protected int[] ReadDimension(Stream buf)
		{
			var tag = new ISMatTag(buf);
		...
		protected string ReadName(Stream buf)
		{
			string s;
		...
		void ReadHeader(Stream buf)
		{
			...
			//br.ReadBytes(8);
			br.BaseStream.Seek(br.BaseStream.Position + 8, SeekOrigin.Begin);
		...
		/// </remarks>
		/// <author>David Zier (<a href="mailto:david.zier@gmail.com">david.zier@gmail.com</a>)</author>
		protected class ISMatTag : MatTag
		{
			/// <summary>
			...
			/// <summary>
			/// padding.  changed by Gyomei 2021/9/1
			/// older code was readonly int padding;
			/// </summary>
			public int Padding { get; protected set; } = 0;
			...
			public ISMatTag(int Type, int Size) :
				base(Type, Size)
			{
			}
			...
			public void ReadToByteBuffer( ByteBuffer buff, IByteStorageSupport storage )
			{
				...
				if (Padding > 0)
					...
					Buf.BaseStream.Seek(Buf.BaseStream.Position + Padding, SeekOrigin.Begin);
			}

			/// <summary>
			/// Read double
			/// Added by Gyomei 2021/9/29
			/// </summary>
			/// <returns></returns>
			public T[] Read<T>()
			{
				var elementSize = SizeOf();
				var elements = _size / elementSize;
				T[] oRcd = new T[elements];
				byte[] btTemp = Buf.ReadBytes(elements * elementSize);
				//block copy, work for Little Endian only??
				Buffer.BlockCopy(btTemp, 0, oRcd, 0, oRcd.Length * elementSize);

				// Ex. using unsafe, if double only
				//unsafe
				//{
				//	fixed (byte* tmp = btTemp)
				//	fixed (double* dest = oRcd)
				//	{
				...
				// Ex. read double directly, if double only
				//for(var i=0; i<oRcd.Length;i++)
				//{
				//	oRcd[i] = Buf.ReadDouble();
				//}
			...
			public string Read()
			{
				...
				var elements = _size / elementSize;
				byte[] btTemp = Buf.ReadBytes(elements * elementSize);
				string strRcd = Encoding.UTF8.GetString(btTemp);
